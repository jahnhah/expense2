...
import type { Member, Transaction, TransactionParticipant, Category } from '@/lib/types';
...
export default function DashboardPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const [membersRes, txRes, catRes] = await Promise.all([
  ...
  const balances = computeMemberBalances(members, transactions, participants);
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <BalanceTable balances={balances} currency={household.currency} />
    </div>
  );
}
